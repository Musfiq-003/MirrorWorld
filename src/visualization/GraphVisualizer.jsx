...
export default function GraphVisualizer({ showTopology = true, activeTool, selectedNodeId, onNodeSelect, themeMode }) {
    const isNight = themeMode === 'night';

    // Dynamic Color Palette
    const THEME = {
        BG: isNight ? '#050508' : '#f1f5f9',
        NODE_DEFAULT: isNight ? '#4f4f6f' : '#94a3b8',
        EDGE_BASE: isNight ? '255, 255, 255' : '0, 0, 0',
        TEXT: isNight ? 'rgba(255, 255, 255, 0.7)' : '#1e293b',
        LEGEND_BG: isNight ? 'rgba(5, 5, 8, 0.8)' : 'rgba(255, 255, 255, 0.9)',
        LEGEND_BORDER: isNight ? 'rgba(255,255,255,0.1)' : 'rgba(0,0,0,0.1)'
    };
            ...
            edges.forEach(edge => {
                ...
                if (showTopology) {
                    ...
                    if (edge.state === 'BLOCKED') {
                        ...
                        ctx.strokeStyle = `rgba(${THEME.EDGE_BASE}, ${0.1 + flowRatio * 0.4})`;
    ...
    return (
        <div ref={containerRef} style={{ width: '100%', height: '100%', position: 'relative', background: THEME.BG, transition: 'background 0.3s ease' }}>
            <canvas
                ref={canvasRef}
                style={{
                    display: 'block',
            ...
            <div
                ...
                style={{
                    ...
                    border: `1px solid ${THEME.LEGEND_BORDER}`,
                ...
                {isLegendMinimized ? (
                    <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                        ...
                        <span style={{ fontSize: '0.7rem', color: THEME.TEXT, fontFamily: 'var(--font-mono)', fontWeight: 700 }}>KEY</span>
                    </div>
                ) : (
                    <>
                        <div style={{ fontFamily: 'var(--font-mono)', fontSize: '0.7rem', color: THEME.TEXT, marginBottom: '0.5rem', textTransform: 'uppercase', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                            <span>Map Key</span>
                            <ChevronDown size={14} />
                        </div>
                        <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
                            {LEGEND_ITEMS.map((item, i) => (
                                <div key={i} style={{ display: 'flex', alignItems: 'center', gap: '8px', fontSize: '0.8rem', color: THEME.TEXT }}>
                                    {item.type === 'line' ? (
                                        <div style={{
                                            width: 12, height: 2,

...
const NODE_ICONS = {
    ...
    [NODE_TYPES.HOUSE]: Home,
    [NODE_TYPES.BUILDING]: Building,
    [NODE_TYPES.WATER]: Waves,
    [NODE_TYPES.CANAL]: Anchor,
    [NODE_TYPES.BEACH]: Umbrella,
    [NODE_TYPES.PARK]: Trees
};
...
export default function ManualControlPanel({
    ...
    useEffect(() => {
        ...
        const update = () => {
            const edge = engine.getEdgeById(selectedEdgeId);
    ...
    const updateNodeProperty = (key, value) => {
        ...
        if (node) {
            node[key] = value;
            if (key === 'capacity') node.maxCapacity = value;
            engine.notify();
        }
    };
    ...
    return (
        <div className="manual-control-panel" style={{
            position: 'absolute',
            top: '80px',
            right: '20px',
            width: '280px',
            ...
            <div style={{ marginBottom: '1.5rem' }}>
                ...
                <div className="tool-grid" style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: '6px', marginBottom: '1rem' }}>
                    {[NODE_TYPES.BRIDGE, NODE_TYPES.WATER, NODE_TYPES.CANAL, NODE_TYPES.BEACH, NODE_TYPES.PARK, NODE_TYPES.WAREHOUSE, NODE_TYPES.INDUSTRIAL].map(type => (
                        <ToolButton
                            key={type}
                            icon={NODE_ICONS[type]}
                            label={type === NODE_TYPES.CANAL ? 'CANAL' : type}
                ...
                <div className="tool-grid" style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: '6px', marginBottom: '1rem' }}>
                    {[NODE_TYPES.HOSPITAL, NODE_TYPES.HOUSE, NODE_TYPES.AMBULANCE].map(type => (
                        <ToolButton
                            key={type}
                            icon={NODE_ICONS[type]}
                ...
                <div className="tool-grid" style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: '6px' }}>
                    {[NODE_TYPES.SOURCE, NODE_TYPES.SINK, NODE_TYPES.VEHICLE, NODE_TYPES.BUILDING].map(type => (
                        <ToolButton
                            key={type}
                            icon={NODE_ICONS[type]}
            ...
            {selectedNode && (
                <div style={{ borderTop: '1px solid rgba(255,255,255,0.1)', paddingTop: '1rem', animation: 'fadeIn 0.2s', marginBottom: '1rem' }}>
                    <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '10px' }}>
                        <div style={{ fontSize: '0.65rem', color: 'var(--accent-primary)', fontWeight: 700 }}>ENTITY_PROPERTIES</div>
            ...
            {selectedEdge && (
                <div style={{ borderTop: '1px solid rgba(255,255,255,0.1)', paddingTop: '1rem', animation: 'fadeIn 0.2s', marginBottom: '1rem' }}>
                    <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '10px' }}>
                        <div style={{ fontSize: '0.65rem', color: 'var(--accent-primary)', fontWeight: 700 }}>ROAD_PROPERTIES</div>
                        <div style={{ fontSize: '0.65rem', color: '#888' }}>ID: {selectedEdge.id.substring(0, 4)}</div>
                    </div>

                    <div style={{ display: 'flex', flexDirection: 'column', gap: '15px' }}>
                        <div>
                            <label style={{ fontSize: '0.7rem', color: '#aaa', display: 'block', marginBottom: '8px' }}>Road Shape / Curvature</label>
                            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: '6px' }}>
                                <button
                                    onClick={() => updateEdgeProperty('curvature', -0.5)}
                                    style={{
                                        background: selectedEdge.curvature === -0.5 ? 'rgba(0, 243, 255, 0.2)' : '#111',
                                        border: selectedEdge.curvature === -0.5 ? '1px solid var(--accent-primary)' : '1px solid #333',
                                        padding: '8px 4px', borderRadius: '4px', color: '#fff', fontSize: '0.65rem', cursor: 'pointer'
                                    }}
                                >
                                    Curve_Left
                                </button>
                                <button
                                    onClick={() => updateEdgeProperty('curvature', 0)}
                                    style={{
                                        background: (selectedEdge.curvature === 0 || !selectedEdge.curvature) ? 'rgba(0, 243, 255, 0.2)' : '#111',
                                        border: (selectedEdge.curvature === 0 || !selectedEdge.curvature) ? '1px solid var(--accent-primary)' : '1px solid #333',
                                        padding: '8px 4px', borderRadius: '4px', color: '#fff', fontSize: '0.65rem', cursor: 'pointer'
                                    }}
                                >
                                    Straight
                        ...
                        <div>
                            <label style={{ fontSize: '0.7rem', color: '#aaa', display: 'block', marginBottom: '4px' }}>Max Capacity</label>
                            <input
                                type="number"
                                value={selectedEdge.maxFlow}
                                onChange={(e) => updateEdgeProperty('maxFlow', parseInt(e.target.value))}
                                style={{ width: '100%', background: '#111', border: '1px solid #333', padding: '6px', borderRadius: '4px', color: '#fff' }}
                            />
                        </div>
                    </div>
